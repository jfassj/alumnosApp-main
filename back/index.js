...
app.post('/alumno/agregar', (req, res) => {
  ...
  const sql =
    'INSERT INTO alumnos (matricula, aPaterno, aMaterno, nombre, sexo, dCalle, dNumero, dColonia, dCodigoPostal, aTelefono, aCorreo, aFacebook, aInstagram, tipoSangre, nombreContacto, telefonoContacto, contrasenha) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';

  bcrypt
    .hash(contrasenha, saltRounds)
    .then(function (hash) {
        ...
        function (err1, result1) {
          if (result1) {
            ...
            const token = jwt.sign(
              {matricula, nombre, aPaterno, aMaterno, foto, permisos},
              'jwtSecret',
              {
                expiresIn: '1 day',
              },
...
app.post('/alumno/photos/upload', verificarToken, (req, res) => {
  ...
  if (auth) {
    ...
    const uploadPath =
      __dirname + '/imagenes/' + matricula + path.extname(archivo.name);
...
app.post('/alumno/mensaje/crear', verificarToken, (req, res) => {
  const {matricula} = req;
  ...
  const sqlGet = 'INSERT INTO alumnos_mensajes values(?,?,?,?)';
  db.query(
    sqlGet,
    [matricula, asunto, cuerpo, hoy.toISOString()],
    (err, result, fields) => {
      if (err) {
        return res.send({
          status: 100,
          resultado: err,
          mensaje:
            'No se pudo contactar al servidor - contacta a tu administrador',
        });
        //console.log(err.errno);
      }
      ...
      if (result.affectedRows > 0) {
        ...
        res.send({
          status: 101,
          resultado: result,
          mensaje: 'El mensaje no fue enviado - contacta a tu administrador',
        });
      }
    },
  );
